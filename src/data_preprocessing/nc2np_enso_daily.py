...
def nc2np_enso_daily(
    ...
    for year in tqdm(years):
        ...
        if name_pattern_year_twice:
            ps = glob.glob(
                os.path.join(path, f"*{year}*{year}*.nc")
        ...
        cropped_dataset_sst = dataset_sst["sst"].sel(
            latitude=slice(max_lat, min_lat), longitude=slice(min_lon, max_lon)
        )
        daily_sst_box[year] = cropped_dataset_sst.resample(time="1D").mean("time")
...
@click.command()
@click.option(
    "--root_dir", type=click.Path(exists=True), default="/p/projects/climate_data_central/reanalysis/ERA5/sst"
)
...
def main(root_dir, save_dir, start_year, end_year, n_days_rolling):

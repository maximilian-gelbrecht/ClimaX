...
def nc2np_daily(
    path, variables, years, save_dir, partition, num_shards_per_year, aggregation_mode
):
    ...
    for year in tqdm(years):
        ...
        for var in variables:
            ...
            if len(dataset[code].shape) == 3:  # surface level variables
                dataset[code] = dataset[code].expand_dims("val", axis=1)
                ...
                for level in all_levels:
                    ds_level = dataset.sel(level=[level])
                    level = int(level)

                    # remove the last 24 hours if this year has 366 days
                    # aggreate to daily data, either by just taking one snapshot or by taking a mean
                    if aggregation_mode == "mean":
                        np_vars[f"{var}_{level}"] = ds_level[code].resample(time='1D').mean('time').to_numpy()[:DAYS_PER_YEAR]
                    elif aggregation_mode == "snapshot":
                        np_vars[f"{var}_{level}"] = ds_level[code][
                            0:HOURS_PER_YEAR:24
                        ].to_numpy()
...
def main(
    ...
    nc2np_daily(
        root_dir, variables, train_years, save_dir, "train", num_shards, aggregation
    )
    nc2np_daily(
        root_dir, variables, val_years, save_dir, "val", num_shards, aggregation
    )
    nc2np_daily(
        root_dir, variables, test_years, save_dir, "test", num_shards, aggregation
    )
